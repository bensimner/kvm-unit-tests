...
static void check_globals(ctx_t *_a) {
  ...
  for (int _i = _a->_p->size_of_test-1 ; _i >= 0 ; _i--) {
    if (rand_bit(&(_a->seed)) && y[_i] != 0) fatal("MP+dmbs, check_globals failed");
    if (rand_bit(&(_a->seed)) && x[_i] != 0) fatal("MP+dmbs, check_globals failed");
  }
...
static void run(cmd_t *cmd,cpus_t *def_all_cpus) {
  ...
  tsc_t start = timeofday();
  schedule(th_ids[0], funs[0], &parg[0]);
  schedule(th_ids[1], funs[1], &parg[1]);
  ...
  hist_t *hist = alloc_hist();
  int outs[2][2] = { {0, 0}, {0, 0} };
  for (int _i = param.size_of_test-1 ; _i >= 0 ; _i--) {
    int _out_1_x0_i = ctx.out_1_x0[_i];
    int _out_1_x2_i = ctx.out_1_x2[_i];
    if (outs[_out_1_x0_i][_out_1_x2_i] == 0) {
        printf("OUT [%d][%d]\n", _out_1_x0_i, _out_1_x2_i);
    }
    outs[_out_1_x0_i][_out_1_x2_i]++;
    outcome_t o;
    int cond;
...
int MP_dmbs(int argc, char** argv);
int MP_dmbs(int argc, char **argv) {
  ...
  cmd_t def = { 1, 1, SIZE_OF_TEST, STRIDE, AVAIL, 0, 0, aff_incr, 0, 1, AFF_INCR, def_all_cpus, NULL, -1, MAX_LOOP, NULL, NULL, -1, -1, -1, 0, 1};
  cmd_t cmd = def;
