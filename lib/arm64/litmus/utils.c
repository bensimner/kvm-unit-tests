...
void pc_wait(pc_t *p) {
  /* assume we hold p->c_mutex */

  pm_lock(p->c_self_mutex);
  pc_wait_t* self = (pc_wait_t*)malloc_check(sizeof(pc_wait_t));
  self->pc_w_next = p->waiters;
  p->waiters->pc_w_prev = self;
  p->waiters = self;
  self->pc_w_prev = NULL;
  self->pc_w_wait = pm_create();
  pm_unlock(p->c_self_mutex);
  pc_unlock(p);
  pm_lock(self->pc_w_wait);
  pm_lock(self->pc_w_wait);

  pm_unlock(self->pc_w_wait);
  pm_free(self->pc_w_wait);
  
  pm_lock(p->c_self_mutex);
  if (self->pc_w_prev != NULL) {
    pc_wait_t* prev = self->pc_w_prev;
    prev->pc_w_next = self->pc_w_next;
  }
  else
    p->waiters = self->pc_w_next;
  pm_unlock(p->c_self_mutex);
...
void pc_broadcast (pc_t *p) {
  ...
  pm_lock(p->c_self_mutex);
  while (p->waiters != NULL) {
    pm_unlock(p->waiters->pc_w_wait);
    p->waiters = p->waiters->pc_w_next;
  }
  pm_unlock(p->c_self_mutex);
}
...
void *join(int cpu) {
  while (threads[cpu].returned == 0);
  return threads[cpu].return_arg;
  /* void *r ; */
